...
class ControllerPaymentSmart2pay extends Controller {
    ...
    public function pay()
    {
        ...
        $methodIDs  = $this->model_payment_smart2pay->getActiveMethods($payment_address);
	...
	if($this->request->get['method'] === '1' || $this->request->get['method'] === '20')
        ...
        foreach ($this->data['payment_data'] as $key => $value) {
            if ( ! $value) {
                unset($this->data['payment_data'][$key]);
            }
        }

        $stringToHash = $this->model_payment_smart2pay->createStringToHash($this->data['payment_data']);

        $this->data['string_to_hash'] = $stringToHash;

        $this->data['payment_data']['Hash'] = $this->model_payment_smart2pay->computeHash($stringToHash, $settings['smart2pay_signature']);
		...
		if($settings['smart2pay_order_confirm'] == 3){
			...
			$this->model_checkout_order->confirm($this->session->data['order_id'], $settings['smart2pay_order_status_new']);
		}	
    ...
    public function feedback()
    {
        ...
        if ( ! $status || ! $order) 
        {
            $this->redirect($server_base . "index.php");
        } 
        ...
        $this->data['feedback'] = $translations['info_payment_feedback_' . $status];
    ...
    private function informCustomer($order)
    {
        ...
        $template->data['order_date'] = date("d F Y", strtotime($order['date_added']));
        $template->data['order_total'] = number_format($order['total'], 2);
        $template->data['order_currency'] = $order['currency_code'];
        $template->data['customer_name'] = $order['firstname'] . " " . $order['lastname'];

        $template->data['suport_email'] = $this->config->get('config_email');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/smart2pay/email/smart2pay_payment_confirmation.tpl')) {
            $html = $template->fetch($this->config->get('config_template') . '/template/smart2pay/email/smart2pay_payment_confirmation.tpl');
        } else {
            $html = $template->fetch('default/template/smart2pay/email/smart2pay_payment_confirmation.tpl');
        }
     ...
     * Check pay request
     */
    private function checkPayRequest($payment_address)
    {
        $this->load->model('payment/smart2pay');
        ...
        if ( ! $payment_address) {
            $error = "Seems like user haven't gone through checkout process or session has been cleared.";
